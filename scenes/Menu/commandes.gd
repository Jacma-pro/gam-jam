...
# Navigation actions (reuse the same names as main_menu)
...
func _ready() -> void:
	# Gestion de la musique persistante (comme dans main_menu)
	...
	if GameManager:
		# In main_menu, CheckButton.button_pressed == sound enabled -> so we mirror that semantics
	...
	# Connect hover/focus SFX like main_menu
	...
	# --- CORRECTION : Forcer les liens de navigation ---
	# On dit explicitement à Godot : "Quand on est sur le slider et qu'on fait BAS, va vers le bouton home"
	if h_slider and home_return:
		# use NodePath values so Godot knows the neighbour control
		h_slider.focus_neighbor_bottom = home_return.get_path()
		# Retour vers le slider depuis le bouton home (si visible)
		home_return.focus_neighbor_top = h_slider.get_path() if h_slider.visible else check_button.get_path()
		# Petit raccourci : aller vers le checkbutton depuis le bouton home à droite
		home_return.focus_neighbor_right = check_button.get_path()

func _on_home_return_pressed() -> void:
	# Clean up persistent music and go back to main menu
...
# Use _unhandled_input to catch inputs that GUI controls might consume
...
func _on_check_button_toggled(toggled_on: bool) -> void:
	# toggled_on true means CheckButton is pressed -> sound enabled -> is_muted = false
	...
	if Input.is_action_just_pressed(nav_ok):
		...
		if focused_control:
			if focused_control == home_return:
				...
				# Ensure UI and GameManager synced
				GameManager.is_muted = not check_button.button_pressed
				_update_audio_server()

	# Navigation: when slider has focus and user presses down, move focus to the home button
	# (nav_up/nav_down handled in _unhandled_input)
